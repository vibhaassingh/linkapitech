...
- **Left for Phase 10 docs:** `createHeroField` is still named in comments in `HeroLens.tsx` L40 and `lensLayout.ts` L17/L75 (both outside this phase's edit permissions) and in `README.md` L50/L124. `layout.mjs` is **not invoked by `gate.sh`** (pre-existing; Part I calls it a hard gate) — it was run standalone for this phase; wiring it in is a gate.sh change for the orchestrator.
