...
- The mobile sheet's rim ring is a `position: absolute; inset: 0` pseudo inside the `overflow-y: auto` sheet, so it sizes to the scrollport and scrolls with the content; on a full-bleed sheet only its top edge is ever visible.
