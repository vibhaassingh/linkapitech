...
| 2 | `.liq*` classes + mobile/coarse/RM blocks; drift tiers; `sheet-shadow`, `pin*`, `.display-0`, `band-hero`, `.lens-*` keyframes; motif kit; `scene/lensLayout.ts`; CursorGlow selector; `contract.py` classes/tokens; `ALLOWED_ANIM="ecoWire conduitPulse"`; new `qa.mjs` ink-on-glass rule | `app/globals.css`, `components/motifs/*`, `components/motion/CursorGlow.tsx`, `scripts/qa/{contract.py,gate.sh,qa.mjs}` | no call sites yet. **Verify `@supports (backdrop-filter: url(#x))` false in Safari + Firefox** before phase 3 |
